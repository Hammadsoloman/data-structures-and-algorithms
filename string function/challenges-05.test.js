...
You are making a grocery list for ingredients needed in the gruffalo crumble recipe, below. Rather than taking the entire recipe,
 you only want a list of the itamNameName names.

Write a function named listFoods that takes in the recipe and returns a new array of the food itamNames without any amount or units. 
...
const listFoods = (recipe) => {
  let result = [];
  recipe.ingredients.forEach((itemName) => {
    let requiredOne = itemName.slice(itemName.indexOf(' ') + 1);
    let requiredTwo = requiredOne.slice(requiredOne.indexOf(' ') + 1);
    result.push(requiredTwo);
  });
  return result;
};
...
const splitFoods = (recipe) => {
  let result = [];
  recipe.ingredients.forEach((itamName) => {
    result.push(itamName.split(' ').splice(2).join(' '));
  });
  return result;
};
...
Write a function named stepAction that takes in the recipe and extracts the action verbs from the steps. Fortunate for you,
the action verbs are the first word of each action.
...
const stepActions = recipe => {
  let result = [];
  recipe.steps.forEach(itemNameName => {
    result.push(itemNameName.split(' ')[0]);
  });
  return result;
};
