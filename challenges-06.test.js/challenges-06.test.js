...
Write a function named hasChildrenValues that uses Object.values to determine if any given character in 
...
const hasChildrenValues = (arr, character) => {
  let resulte = false;
  arr.forEach(() => {
    if (indiv.name === character) {
      resulte = indiv.children.length > 0;
    }
  });
  return resulte;
};
